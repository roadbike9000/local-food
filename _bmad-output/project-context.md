...
- Stripe 16.12 — **hosted Checkout only**, never Elements/PaymentIntents (keeps the app out of PCI scope). Webhook route reads raw `.text()` body — never `.json()` before `constructEvent`, or signature verification breaks. Replay-guarded via two independent one-shot flags: `smsNotified` (SMS) and, as of Story 1.4, `Order.stockDecremented` for stock decrement. `stockDecremented` is deliberately a *separate* flag from `status` (the `PENDING → PAID` transition itself is guarded by `updateMany({ where: { status: "PENDING" } })`) — this lets a retry re-attempt the decrement if a previous call flipped `status` to `PAID` but then failed before finishing (deadlock, dropped connection). It's **claimed atomically as the first write inside the decrement's own `$transaction`**, not read-then-acted-on — a plain read-then-act version of this exact flag shipped briefly and let two *concurrent* deliveries of the same event both decrement (caught in round-2 review, never released). A stock shortfall is terminal by design: the claim survives a shortfall (only the specific decrements roll back), so a later replay never silently re-decrements once stock is replenished. `smsNotified`, by contrast, is still a plain read-then-act check — safe against sequential replay only, not concurrent. Neither guard dedupes on the Stripe event ID itself, so don't assume replay-safety for anything new added to this route beyond what these two flags already cover, and don't copy `smsNotified`'s pattern for anything that needs concurrent-safety — copy the atomic-claim pattern instead
...
- **A literal/hardcoded date-time string in a test fixture is correct only until real time catches up to it — then it silently starts failing (or silently starts passing for the wrong reason) with no commit marking the moment it broke.** Shipped twice: `prisma/seed.ts`'s "tomorrow" pickup slot, computed once relative to whenever `db:seed` last ran, drifted stale and broke `GET /api/vendors/[vendorId]/pickup-slots` (caught during Story 5.1's own manual verification, not by review); `src/app/api/pickup-slots/schema.test.ts`'s `validBody` hardcoded `startsAt: "2026-08-10T..."` on 2026-08-09 (`git blame` confirms — it was "tomorrow" *then*), discovered stale while authoring Story 5.2, over two weeks after it quietly became a ticking time bomb. **Diff-scoped code review structurally cannot catch this class of bug** — the literal was genuinely correct in the commit that introduced it, so nothing in that diff ever looked wrong; only wall-clock time passing makes it wrong, invisibly, with no diff to review at that moment. Any fixture asserting behavior relative to "now" (upcoming, expired, already-past, still-valid) must compute its dates relative to `Date.now()`/`new Date()` at run time, never a frozen ISO literal — this applies to seed scripts and test fixtures alike
