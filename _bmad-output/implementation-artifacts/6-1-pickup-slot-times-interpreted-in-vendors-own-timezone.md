...
- Full regression: typecheck clean, lint clean, 108/108 unit, 137/137 e2e (zero skips, zero regressions across the entire suite — not just the touched files), production build succeeds.

### File List

- `prisma/schema.prisma` (modified — `Vendor.timezone`)
- `prisma/migrations/20260827172107_add_vendor_timezone/migration.sql` (new)
- `src/lib/timezone.ts` (new — implemented for real, replacing the ATDD-pass stub)
- `src/lib/timezone.test.ts` (from the ATDD pass, unmodified — all 11 cases now pass against the real implementation)
- `src/lib/utils.ts` (modified — `formatPickupWindow()` gained a required `timeZone` parameter)
- `src/lib/utils.test.ts` (modified — existing test updated to pass the runtime's resolved zone; 1 new test proving the parameter changes output)
- `src/components/dashboard/AddSlotForm.tsx` (modified — `timezone` prop, `zonedWallTimeToUtc`/`utcInstantToZonedDatetimeLocal` call sites, removed the now-redundant local `toDatetimeLocalValue()`, added a pre-conversion input-shape guard)
...
- `src/app/cart/page.tsx` (modified, not originally planned — `vendorTimezone` state, passes it to both `formatPickupWindow` call sites)
- `src/app/api/vendors/[vendorId]/pickup-slots/route.ts` (modified, not originally planned — response now also includes `timezone`)
- `tests/helpers/db.ts` (modified — `createTestVendor()` gained a `timezone` override)
- `tests/dashboard.spec.ts` (modified — 1 new e2e test proving AC #2/#4 end-to-end)
