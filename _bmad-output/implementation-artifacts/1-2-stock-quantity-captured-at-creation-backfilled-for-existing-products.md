...
- `src/lib/inventory.test.ts` (new — unit coverage for `setStock()`'s optimistic lock and `setLowStockThreshold()`'s marker-clearing, against the real DB)
- `src/lib/inventory.migration.test.ts` (new — pins the original backfill migration's literals to the named constants)
...
- `vitest.config.mts` (modified — loads `.env` into `process.env` via Vite's `loadEnv`, so Prisma-backed unit tests can run)
