...
5. **Given** `DESIGN.md#Do's and Don'ts` documents three known mock-drift spots (a near-duplicate `sold-out-bg` hex value, a couple pixels of spacing drift between the vendor-page and cart mocks, and the cart's price typography rendering 16px against the canonical 15px `{typography.price}` token)
   **When** this story ships
...
- [x] Task 1: Extend the Tailwind token layer (AC #1, #5)
  - [x] `tailwind.config.ts` — add `DESIGN.md`'s full color palette to `theme.extend.colors` (19 tokens: `cream`, `cream-deep`, `paper`, `card-border`, `line`, `field-border`, `terracotta`/`terracotta-deep`/`terracotta-light`, `olive`/`olive-deep`/`olive-light`, `sage-light`, `selected-wash`, `sold-out-bg`, `disabled-outline`, `placeholder-text`, `ink`, `ink-soft`) — keep the existing `brand` key too (still referenced by untouched pages until Epic 8's later stories migrate them; don't delete it in this story).
...
- **Code review round 1, low-severity fixes:**
  - Cart badge overflow: at 3+ digit counts the fixed `h-5 w-5` circle would clip/squish the number. Badge now caps its visible text at `"99+"` (`count > 99`) and the pill sizes with `min-w-5 px-1` instead of a fixed width — the full count stays exact in `aria-label`, only the visible glyph is capped. Unreachable with today's real stock ceilings, but not guarded before.
  - No `aria-live` on cart-count changes: adding an item updates the link's `aria-label`, but attribute changes on their own aren't reliably announced by screen readers — added a `sr-only` `aria-live="polite"` region alongside the cart-pill that announces `"Cart, {n} items"` on count change (empty at 0 so mount is silent).
  - Lost hover state: the old plain-text cart link had `hover:text-brand`; the new pill had no hover treatment at all. Added `hover:border-terracotta`.
  - AC #5 undercounted `DESIGN.md`'s documented mock-drift spots at two (`sold-out-bg`, spacing) when `DESIGN.md#Typography` documents a third — the cart mock's price rendering at a drifted 16px against the canonical 15px `{typography.price}`. Implementation already used the canonical 15px (`fontSize.price` in `tailwind.config.ts`), so this was a documentation-completeness gap only, not a code bug — AC #5 and Task 1 updated to name all three.
  - Doc-count drift: Task 1's checklist said "17 tokens" for the color palette (actual 19, matching the Completion Notes' own correct count) and "19 typography roles" appeared in two places against `DESIGN.md`'s actual 20 (verified by counting `DESIGN.md#Typography`'s frontmatter keys directly) — both corrected to match the real counts.
  - `ProductCard.tsx`'s `ProductImagePlaceholder` inline `<svg>` (pre-dates this story, Story 4.2) duplicated `icon-line`'s conventions as a second, separately-maintained copy instead of living in the new shared `Icons.tsx`. Moved into `Icons.tsx` as `ImagePlaceholderIcon` (using the same `mergeIconProps` merge as the other five icons); `ProductCard.tsx` now imports it instead of inlining its own `<svg>`.
