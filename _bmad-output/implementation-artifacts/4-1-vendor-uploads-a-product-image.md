...
- **This Cloudinary account is not configured** — `.env`'s `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` hold short placeholder values (17/5/5 chars), not real credentials. Confirmed via a direct probe outside the app (`cloudinary.uploader.upload()` called standalone against the fixture image): `cloud_name is disabled`, HTTP 401. Confirmed with the user mid-session — they haven't set up a Cloudinary account yet. Per Testing Standards' documented decision path (mirror `payment.spec.ts`'s Stripe-key-skip pattern), `test.skip()`'d the two scenarios that require a real upload: `tests/products-api.spec.ts`'s happy-path 200 case and `tests/dashboard.spec.ts`'s UI upload case. Every other scenario (schema validation, auth, 400/401 paths, oversized-file rejection) needs no real Cloudinary call and runs for real. Un-skip both once `.env` has working `CLOUDINARY_*` values.
...
- `npx playwright test tests/products-api.spec.ts` — 8/8 passed, 1 skipped (Cloudinary happy-path, see above).
- `npx playwright test tests/dashboard.spec.ts` — 20/20 passed, 1 skipped (Cloudinary happy-path, see above).
- Full `npx playwright test` — 118 passed, 2 skipped (both the Cloudinary-dependent cases above; no other regressions).
...
- **Cloudinary is not configured in this environment** (placeholder `.env` values, confirmed with the user) — 2 of the story's ATDD scenarios are `test.skip()`'d with a documented reason and un-skip instructions; every other scenario (11 of 13) runs for real and passes. This is an environment/credentials gap, not a code defect — the route's error-handling path (502 on a real Cloudinary failure) was incidentally exercised for real during diagnosis.
...
- Full regression: typecheck clean, lint clean, 86/86 unit, 118/120 e2e (2 expected skips, both Cloudinary-dependent, documented above), no regressions in any pre-existing suite.
...
- `tests/products-api.spec.ts` (modified — new `POST /api/products/upload-image` describe block activated; unauthenticated case restructured to a standalone anonymous test; happy-path case skipped pending real Cloudinary credentials)
- `tests/dashboard.spec.ts` (modified — oversized-image case activated; happy-path upload case skipped pending real Cloudinary credentials)
