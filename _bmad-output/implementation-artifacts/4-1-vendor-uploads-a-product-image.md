...
- [x] [Review][Patch] Cloudinary host check doesn't scope to this app's own cloud — `CreateProductSchema.imageUrl`'s `.refine()` only checks the `res.cloudinary.com` hostname, not this app's own `CLOUDINARY_CLOUD_NAME` path segment. Any vendor can `POST /api/products` directly with any public Cloudinary asset URL from *any* account. **Decision (2026-08-24, user):** tighten to scope to this app's own cloud name — `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/`. [src/app/api/products/schema.ts:14-17]
- [x] [Review][Patch] Client-side and server-side image-size caps don't actually agree, contradicting Dev Notes' own "matches server-side cap" claim — `AddProductForm.tsx`'s `MAX_IMAGE_BYTES = 3 * 1024 * 1024` (3,145,728 bytes) vs. `UploadImageSchema`'s `MAX_BASE64_LENGTH = 4_000_000` chars (≈2,999,983 raw bytes, decimal 3MB not binary 3MiB). A file in the ~140KB band between those two numbers passes both client-side checks, gets uploaded, and only then gets rejected server-side as "too large" [src/components/dashboard/AddProductForm.tsx:17, src/app/api/products/upload-image/schema.ts:5]
- [x] [Review][Patch] `UploadImageSchema`'s base64 format check only validates the data-URL prefix, not the payload — an empty or malformed payload (e.g. `data:image/png;base64,` or non-base64 characters) passes schema validation and reaches `uploadImage()`, surfacing as an opaque 502 instead of a clear 400 [src/app/api/products/upload-image/schema.ts:9-12]. **Incident during this fix:** the first attempt used a regex (`/^data:image\/(...);base64,[A-Za-z0-9+/]+={0,2}$/`) tested against the full payload string — under full-suite parallel load, this reproducibly threw `RangeError: Maximum call stack size exceeded` on the ~4.19MB oversized-payload test (V8 regex backtracking recursion on a very long greedy match), turning an expected 400 into a real 500. Reproduced 3/3 full-suite runs before the fix, confirmed clean 2/2 after. Fixed by replacing the payload-content check with a plain character-by-character loop (`hasValidBase64Payload()`), which has no recursion risk regardless of input length.
- [x] [Review][Patch] New upload route's auth-check block was copied without the rationale comment explaining why the explicit `assertVendorActive()` check is needed (Epic 2 retro tech debt — `getCurrentVendor()` doesn't filter deactivated vendors) — every other vendor route in this codebase carries that comment, this one doesn't [src/app/api/products/upload-image/route.ts:19-29]
- [x] [Review][Patch] New Cloudinary-dependent E2E tests have no credential skip-guard — `CLOUDINARY_*` is absent from `.github/workflows/ci.yml`'s secrets (confirmed by reading the workflow file), so the happy-path upload tests will hard-fail, not skip, the first time this branch's tests run in CI [tests/products-api.spec.ts, tests/dashboard.spec.ts]
- [x] [Review][Patch] A genuinely empty (0-byte) selected file is silently treated as "no image chosen," with zero feedback to the vendor — product gets created with no image and no explanation [src/components/dashboard/AddProductForm.tsx handleSubmit's `hasImage` check]
- [x] [Review][Patch] The "too large" inline error set by `handleImageChange` is never cleared if the vendor subsequently selects a valid file — the stale error message lingers on screen until submit [src/components/dashboard/AddProductForm.tsx handleImageChange]
- [x] [Review][Patch] No test locks in the documented distinct error-message text (format-rejection vs. size-cap-rejection) — both the malformed-value and oversized-payload tests assert status code only, never message content, despite `api-contracts.md` and the route's own comments making a point of the two messages being deliberately distinct [src/app/api/products/upload-image/schema.test.ts, tests/products-api.spec.ts]
- [x] [Review][Defer] No cleanup for orphaned Cloudinary uploads if `POST /api/products` fails after the image upload already succeeded [src/components/dashboard/AddProductForm.tsx] — deferred, pre-existing design tradeoff explicitly discussed in this story's own Dev Notes ("Sequencing: upload, then create"); no compensating-transaction convention exists elsewhere in this codebase to reuse
- [x] [Review][Defer] Live Cloudinary E2E happy-path test uploads a real (tiny, 68-byte) image every run with no cleanup [tests/products-api.spec.ts] — deferred, matches this codebase's established no-mocking-external-services convention (Stripe/Clerk/Twilio tests all hit real services too); storage cost is negligible
