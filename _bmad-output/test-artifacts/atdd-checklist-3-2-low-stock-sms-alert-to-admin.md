...
- `npx playwright test tests/inventory.spec.ts tests/webhooks.spec.ts --list` — 44 Playwright tests total (35 pre-existing + 9 new; the other 2 of this story's 11 scaffolds are Vitest, not part of this count) all parse and list correctly; `tsc` errors inside `test.skip()` bodies don't block Playwright's own listing, matching the established pattern from every prior story's scaffolds. (Round-1 review added a 10th Playwright case — a regression test for a real bug found during review — bringing the current total to 45; not reflected in this checklist's original count, which describes the state at ATDD generation time.)
