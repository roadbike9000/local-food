 ...
 *
 * Stripe calls this URL after events happen. We handle three:
 *   - checkout.session.completed: fires immediately for every session,
 *     including delayed-notification payment methods that haven't
 *     actually captured money yet (payment_status "unpaid" in that case).
 *     Only proceeds to mark the order paid if payment_status === "paid" -
 *     otherwise waits for one of the two events below (deferred-work.md:
 *     this event alone never guaranteed money was captured before this
 *     check existed).
 *   - checkout.session.async_payment_succeeded: fires when a delayed
 *     method's payment actually clears. Runs the exact same success path
 *     as an immediately-paid checkout.session.completed.
 *   - checkout.session.async_payment_failed: fires when a delayed method's
 *     payment definitively fails. Moves a still-PENDING order to
 *     CANCELLED; never marks paid, never decrements stock (nothing to
 *     reverse, since a decrement never happened on this path).
 *
 * Shared success logic (mark PAID, decrement stock, text the customer):
 *   1. Only a genuinely PENDING order transitions to PAID - a replay or an
 *      order that has since moved to READY/COMPLETED/CANCELLED is never
 *      touched.
 *   2. Decrement stock for each line item (Story 1.4). Guarded by an
...
// Shared by checkout.session.completed (when already paid) and
// checkout.session.async_payment_succeeded (a delayed method that just
// cleared) - both carry the same Checkout Session shape and mean the same
// thing: money has genuinely been captured for this order.
async function handlePaymentSucceeded(session: Stripe.Checkout.Session) {
  ...
  // Bad/stale orderId - nothing to do. Returning early here (instead of
  // the old bare .update()'s uncaught P2025-turned-500) is an incidental
...
// checkout.session.async_payment_failed: a delayed-notification payment
// method's payment definitively failed (deferred-work.md - previously
// unhandled entirely). Money was never captured on this path, so there's
// no stock to reverse. Only a still-PENDING order transitions - same
// "never pull back from READY/COMPLETED/CANCELLED, never touch an order
// that already became PAID some other way" reasoning as the success path.
async function handlePaymentFailed(session: Stripe.Checkout.Session) {
  ...
  if (
    ...
    console.error(
      "[webhooks/stripe] session.id mismatch on async_payment_failed, ignoring event",
      orderId,
    );
    return;
  }
...
export async function POST(req: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // A delayed-notification payment method completes the session before
    // money actually clears (payment_status "unpaid") - wait for
    // async_payment_succeeded/failed instead of acting now.
