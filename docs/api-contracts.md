...
6. Loads the order's `items` regardless of whether this call made the transition (a retry after a transient failure needs to see them too — see step 6a). If any line finds insufficient stock (a post-payment race — money already captured, stock ran out before this webhook ran), the whole transaction rolls back, every short line (not just the first) is reported via `Sentry.captureException` + `console.error` with the order id and each short product's requested/available quantities, and the order still ends up `PAID` — there's nothing to "reject" once Stripe has already charged the customer. This is not auto-resolved or auto-refunded; it's surfaced for manual admin follow-up (Story 3.2, once an Admin identity exists).
   ...
   6b. **Story 3.2 — admin alerts, sent after the transaction commits (never inside it — an SMS is an external network call and must not run while holding DB row locks).** Two independent cases, both can fire from the same webhook call if a multi-item order has one line cross its threshold while another line shortfalls:
       - **Low-stock crossing:** for each product whose decrement newly brought it to/below `lowStockThreshold` (`decrementStock()`'s `crossedLowStock`, `false` if `lowStockAlerted` was already `true` — "newly crossed," not just "currently low"), `getAdminPhoneNumbers()` (`src/lib/admin.ts` — every `Admin` row with a phone set, not just one) is fetched once, then a low-stock SMS is sent to each phone. `Product.lowStockAlerted` is set `true` only if at least one send succeeds — mirrors `smsNotified`'s "never mark delivered before it's sent" rule. No atomic claim needed for this write: `crossedLowStock` was already computed inside a transaction gated by the outer `stockDecremented` claim (step 6a), which guarantees only one concurrent delivery of a given event ever reaches this code path.
       - **Shortfall:** for each shortfalled line (same set step 6 already reports via Sentry/`console.error`), a *distinct* shortfall SMS is sent to every admin phone. **Never gated by `lowStockAlerted`** — a shortfall means a customer's payment was already captured and the order couldn't be fully fulfilled, a different and more urgent event class than the routine low-stock crossing; each occurrence is surfaced every time, not deduped.
       - Zero admins with a phone configured → no SMS attempted, `lowStockAlerted` stays unchanged. Expected in any environment with no admin phone set up, not an error.
